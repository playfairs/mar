...
searchEntries connection term filters
  ...
  where
    ...
    ftsSql =
        Query
            ( Text.pack
                ( "SELECT "
                    ++ columns
                    ++ " FROM entries e JOIN entries_fts f ON f.rowid = e.rowid WHERE f.entries_fts MATCH ?"
                    ++ clauses
                    ++ " ORDER BY e.path"
                )
            )
    likeSql =
        Query
            ( Text.pack
                ("SELECT " ++ columns ++ " FROM entries e WHERE (e.name LIKE ? OR e.path LIKE ?)" ++ clauses ++ " ORDER BY e.path")
            )
