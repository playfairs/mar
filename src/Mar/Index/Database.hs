...
insertEntry connection entry = execute connection (Query "INSERT INTO entries(path,name,extension,parent,kind,size,modified_at,created_at,permissions,hidden,symlink_target) VALUES (?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(path) DO UPDATE SET name=excluded.name, extension=excluded.extension, parent=excluded.parent, kind=excluded.kind, size=excluded.size, modified_at=excluded.modified_at, created_at=excluded.created_at, permissions=excluded.permissions, hidden=excluded.hidden, symlink_target=excluded.symlink_target") params
  where
    params = [toField (entryPath entry), toField (entryName entry), toField (entryExtension entry), toField (entryParent entry), toField (show (entryType entry)), toField (entrySize entry), toField (entryModified entry), toField (entryCreated entry), toField (entryPermissions entry), toField (if entryIsHidden entry then (1 :: Int) else 0), toField (entrySymlinkTarget entry)]

entriesUnder :: Connection -> FilePath -> IO [Entry]
entriesUnder connection path = query connection "SELECT path,name,extension,parent,kind,size,modified_at,created_at,permissions,symlink_target,hidden FROM entries WHERE parent = ? ORDER BY kind, name" (Only path)

entriesInRoot :: Connection -> FilePath -> IO [Entry]
entriesInRoot connection root = query connection "SELECT path,name,extension,parent,kind,size,modified_at,created_at,permissions,symlink_target,hidden FROM entries WHERE path = ? OR path LIKE ? ORDER BY path" (root, root ++ "/%")

indexedEntry :: Connection -> FilePath -> IO (Maybe Entry)
indexedEntry connection path = do
    entries <- query connection "SELECT path,name,extension,parent,kind,size,modified_at,created_at,permissions,symlink_target,hidden FROM entries WHERE path = ?" (Only path)
...
addRoot connection path = do
    ...
    now <- getCurrentTime
    if exists then pure False else execute connection "INSERT INTO roots(path, added_at) VALUES (?, ?)" (path, now) >> pure True
...
countEntries connection = do
    [(files, directories, total)] <- query_ connection "SELECT coalesce(sum(kind = 'RegularFile'), 0), coalesce(sum(kind = 'Directory'), 0), count(*) FROM entries"
...
markUpdated connection = do
    now <- getCurrentTime
    execute connection "INSERT INTO metadata(key, value) VALUES ('last_update', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value" (Only now)
